...
class DocumentLoader:
    ...
    def load_markdown_file(self, file_path: str, filename: str | None = None) -> list[dict]:
        """
        Load a single Markdown file as one logical page (page_number=0) and run three-level chunking.
        """
        ...
        try:
            ...
            raise Exception(f"处理 Markdown 失败: {str(e)}") from e

    def load_text_file(self, file_path: str, filename: str | None = None) -> list[dict]:
        """
        Load a plain text file (.txt or similar) as one logical page (page_number=0).
        Uses the same three-level chunking pipeline as Markdown (same splitter params).
        """
        path = os.path.abspath(file_path)
        fname = filename or os.path.basename(path)
        try:
            ...
            raise Exception(f"处理文本文件失败: {str(e)}") from e
