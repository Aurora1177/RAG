...
from .llm_env import API_KEY, MODEL, BASE_URL, GRADE_MODEL, OPENAI_COMPAT_EXTRA_BODY
...
GRADE_PROMPT = (
    ...
    # DashScope requires the word "json" in messages when using json_object response_format (structured output).
    "Reply with JSON only: an object with field binary_score ('yes' or 'no')."
)
...
def rewrite_question_node(state: RAGState) -> RAGState:
    ...
    if router:
        prompt = (
            ...
            # DashScope: messages must mention "json" when using json_object / structured output.
            "Reply with JSON only: an object with field strategy, one of step_back, hyde, complex.\n"
            f"用户问题：{question}"
        )
        try:
