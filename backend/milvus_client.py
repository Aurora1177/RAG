...
class MilvusManager:
    ...
    def __init__(self):
        ...
        uri_override = (os.getenv("MILVUS_URI") or "").strip()
        local_path = (os.getenv("MILVUS_LOCAL_PATH") or "").strip()
        if uri_override:
            self.uri = uri_override
        elif local_path:
            p = Path(local_path)
    ...
    def count_entities_by_filename(self, filename: str) -> int:
        """Count stored rows (e.g. leaf chunks) for a logical filename."""
        if not filename:
            return 0
        safe = filename.replace("\\", "\\\\").replace('"', '\\"')
        rows = self.query_all(
            filter_expr=f'filename == "{safe}"',
