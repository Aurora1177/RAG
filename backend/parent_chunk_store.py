"""Parent chunk store for Auto-merging (SQLite + memory cache)."""
...
class ParentChunkStore:
    ...
    def list_distinct_filenames(self) -> List[dict]:
        """Return [{filename, file_type}, ...] for documents that have parent chunks stored."""
        db = SessionLocal()
        try:
            rows = db.query(ParentChunk.filename, ParentChunk.file_type).all()
            seen: set[str] = set()
            out: List[dict] = []
            for fname, ftype in rows:
                if not fname or fname in seen:
                    continue
                seen.add(fname)
                out.append({"filename": fname, "file_type": (ftype or "")})
