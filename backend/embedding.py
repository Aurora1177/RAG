...
_DEFAULT_STATE_PATH = _ROOT / "data" / "bm25_state.json"
...
class EmbeddingService:
    ...
    def __init__(self, state_path: Path | str | None = None):
        _m = os.getenv("EMBEDDING_MODEL", "BAAI/bge-m3")
        _d = os.getenv("EMBEDDING_DEVICE", "cpu")
        print(
            f"[embedding] Loading dense model {_m} on {_d} "
            "(first run may download weights from HuggingFace; CPU encoding is slow for large documents)."
        )
        self._embedder = _create_dense_embedder()
        print("[embedding] Dense embedder ready.")
...
def get_embedding_service() -> EmbeddingService:
    """Lazy singleton so FastAPI can boot before the HF model finishes downloading."""
...
# Shared by ingest and retrieval; BM25 state file is loaded on first real use.
